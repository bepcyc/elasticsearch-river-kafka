...
public class KafkaClient {
	...
	private PartitionMetadata findLeader(List<Map<String, Object>> seedBrokers, String topic, int partition) {
		...
		loop: for (Map<String, Object> seed : seedBrokers) {
			...
			try {
				...
				logger.warn("Error communicating with Broker [" + seed + "] to find Leader for [" + topic + ", " + partition + "] Reason: " + e);
			} finally {
	...
	public long getOffset(String riverName, String topic, int partition, long offset) {
	...
	ByteBufferMessageSet fetch(String topic, int partition, long offset, int maxSizeBytes) {
		FetchRequest req = new FetchRequestBuilder().clientId(clientName).addFetch(topic, partition, offset, maxSizeBytes).build();
